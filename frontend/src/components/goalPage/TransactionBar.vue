    ...
    methods: {
      generateChart() {
        ...
        const totalChangeData = labels.map(date => {
          ...
          const dailyTotal = dailyTransactions.reduce((sum, transaction) => sum + transaction.amount, 0);
          previousBalance += dailyTotal;
        ...
        if (elements.length > 0) {
          ...
          if (datasetIndex === 0) { // Receiving column
            this.selectedTransactions = this.transactions.filter(transaction => 
              new Date(transaction.date).toLocaleDateString() === label && transaction.amount > 0
            );
