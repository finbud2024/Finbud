                  ...
                  <table>
                    ...
                    <tbody v-if="paginatedStockQuotes.length">
                      ...
                      >
                        ...
                        <td data-label="Previous Close">
                          {{ stock["08. previous close"] }}
                        </td>
                        <td data-label="Change">{{ stock["09. change"] }}</td>
                        <td data-label="Change Percent">
                    ...
                    <table>
                      ...
                      <tbody v-if="paginatedRealEstate.length">
                        ...
                        >
                          <td data-label="Type">
                            {{
                              estate.propertyType && !isNaN(estate.propertyType)
                                ? estate.propertyType
                                : "Single-family"
                            }}
                          </td>
    ...
    async generateOverallMarketSummary() {
      try {
        ...
        const lines = [
          ...
          ...cryptos.map(
            (c) =>
              `• ${c.symbol}: $${c.price.toFixed(2)} (${c.change.toFixed(2)}%)`
          ),
          "**Real Estate**",
          ...estates.map(
            (e) =>
              `• ${e.propertyType} at ${
                e.formattedAddress
              }: $${e.lastSalePrice.toLocaleString()}`
          ),
        ].join("\n");
