        ...
        <div class="inline-label-select">
          <label
            ...
            <select v-model="filterType" class="select-box">
              <option v-if="selectedTable === 'GDP'" value="quý">
                {{ t("macroEcon.quarter") }}
              </option>
            ...
            <div class="inline-label-select">
              <label
                ...
                <select v-model="fromQuarter" class="select-box">
                  <option v-for="q in Quarters" :key="q" :value="`Quý ${q}`">
                    {{ t("macroEcon.quarter") }} {{ q }}
                  </option>
                </select>
              </label>
            </div>
            ...
            <div class="inline-label-select">
              <label
                ...
                <select v-model="fromMonth" class="select-box">
                  <option v-for="m in Months" :key="m" :value="`Tháng ${m}`">
                    {{ t(`macroEcon.enMonth.${m}`) }}
                  </option>
                </select>
              </label>
            </div>
        ...
        <thead>
          <tr>
            <th
              ...
              :class="{
                'sticky-col':
                  (index === 0 || index === 1) && selectedTable !== 'Tổng quan',
            ...
            <td
              ...
              :class="{
                ...
                'sticky-col':
                  ...
                  selectedTable !== 'Tổng quan',
                'sticky-first':
                  cellIndex === 0 && selectedTable !== 'Tổng quan',
                'sticky-second':
                  cellIndex === 1 && selectedTable !== 'Tổng quan',
              }"
            >
              {{ cell }}
            </td>
...
const header = computed(() => ({
  ...
  GDP: [t("macroEcon.header.Chỉ tiêu"), t("macroEcon.header.Đơn vị tính")],
  FDI: [t("macroEcon.header.Chỉ tiêu"), t("macroEcon.header.Đơn vị tính")],
  CPI: [t("macroEcon.header.Chỉ tiêu"), t("macroEcon.header.Đơn vị tính")],
  "Xuất-Nhập khẩu": [
    t("macroEcon.header.Chỉ tiêu"),
    t("macroEcon.header.Đơn vị tính"),
  ],
...
const tableSources = {
  ...
  "Xuất-Nhập khẩu":
    ".netlify/functions/server/api/vietstock/ImportExport/filter/",
};
        ...
        data.forEach((row) => {
          ...
          if (row.latestData?.chart) {
            ...
            if (temp.includes("Tháng")) {
              const month = t(
                `macroEcon.enMonth.${temp.match(/Tháng (\d+)\//)[1]}`
              );
              const year = temp.match(/\/(\d{4})$/)[1];
              temp = `${month} ${year}`;
            } else if (temp.includes("Quý")) {
              const quarter = t(
                `macroEcon.enQuarter.${temp.match(/Quý (\d+)\//)[1]}`
              );
              const year = temp.match(/\/(\d{4})$/)[1];
              temp = `${quarter} ${year}`;
            }
      ...
      if (response) {
        ...
        if (fType === "month") {
          rows.headers.forEach((header) => {
            const month =
              fType === "month"
                ? t(`macroEcon.enMonth.${header.match(/Tháng (\d+)\//)[1]}`)
                : "";
            const year = header.match(/\/(\d{4})$/)[1];
            if (fType === "month") headers.push(`${month} ${year}`);
          ...
          for (let i = 0; i < row.value.length; i++) {
            ...
            if (data.category) {
              if (result[i].length == 0) {
                result[i].push(
                  t(`macroEcon.${tName}.${fType}.${data.category}`)
                );
                result[i].push("");
                result[i].push("");
              } else result[i].push("");
              ...
              if (result[i].length === 0) {
                result[i].push(
                  t(`macroEcon.${tName}.${fType}.${data.chi_tieu}`)
                );
...
body[data-route*="macro-economic"]
  .chatBubble
  .chatBubbleContainer
  .chatBubbleHeader {
