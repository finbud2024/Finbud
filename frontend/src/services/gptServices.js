...
export async function gptServices(payload) {
  const defaultSystemMessage = {
    ...
    content: `You are FinBud, a chill, friendly, and reliable financial assistant for Gen Z and Gen Alpha.
Detect the language of the user's input and respond in the same language.
Speak in a young, casual, and fun way... (etc)`,
  };
  ...
  try {
    const response = await axios.post(
      ...
      {
        model: "deepseek/deepseek-chat:free", // ✅ Corrected
        messages: fullMessages,
        temperature: 0.7,
        max_tokens: 1000,
      },
      {
        headers: {
          Authorization: `Bearer ${process.env.VUE_APP_DEEPSEEK_API_KEY}`, // ✅ Or VITE_ prefix if using Vite
          "Content-Type": "application/json",
          "Accept": "application/json", // ✅ Add this just in case
        },
      }
    );
    ...
    console.error("Error in generating response in gptServices:", err.message);
    throw err;
  }
};
