<<<<<<< Updated upstream:backend/Endpoints/postRoute.js
...
import express from "express";
import mongoose from "mongoose";
import Post from "../../database-schema/social/Post.js";
import Forum from "../../database-schema/social/Forum.js";
import User from "../../database-schema/core/User.js";
import ScrapedUser from "../../database-schema/market-data/ScrapedUser.js";
import { isAuthenticated } from '../../middleware/auth.js';
...
postRouter.get("/post/:postId", isAuthenticated, async (req, res) => {
  try {
    ...
    const post = await Post.findById(postId).populate("forumId");
    if (!post) return res.status(404).json({ error: "Post not found" });
...
postRouter.post("/post/:postId/add-comment", isAuthenticated, async (req, res) => {
  try {
    const { postId } = req.params;
    const { body, userId, userModel } = req.body;

    if (!userId || !userModel) return res.status(400).json({ error: "Missing user ID or userModel" });
...
postRouter.post("/", isAuthenticated, async (req, res) => {
  try {
    const { forumId, title, body, userId, userModel } = req.body;

    if (!userId || !userModel) return res.status(400).json({ error: "Missing user ID or userModel" });
    if (!forumId || !title.trim() || !body.trim()) return res.status(400).json({ error: "Missing required fields" });
...
export default postRouter;
>>>>>>> Stashed changes:backend/Endpoints/social/postRoute.js
