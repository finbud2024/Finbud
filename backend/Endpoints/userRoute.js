...
userRoute.route('/users/:userId')
    ...
    .get(async(req,res)=>{
		...
		console.log('in /threads Route (GET) thread with ID:' + JSON.stringify(userId));
		try{
			let user = await User.findOne({"_id": UserId});
			if(!user){
				return res.status(404).send("No user with id: " + JSON.stringify(userId) + "existed in database");
			}
			return res.status(200).json(user);
		}catch(err){
			return res.status(404).send("Unexpected error occured when looking for user with id: "+ userId + "in database: "+ err);
		}
    })
