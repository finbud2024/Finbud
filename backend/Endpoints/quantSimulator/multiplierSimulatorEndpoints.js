...
// Helper function to get Socket.io instance from the Express app
const getSocketIo = (req) => {
  const io = req.app.get('simulatorIo');
  if (!io) {
    console.warn('Socket.io instance not found in app context');
  }
  return io;
};

// Helper function to emit events to simulation room
const emitToSimulation = (req, sessionId, event, data) => {
  const io = getSocketIo(req);
  if (io) {
    // Emit to the specific simulation room
    const roomId = `simulation:${sessionId.split(':')[1]}`;
    io.to(roomId).emit(event, data);
    console.log(`Emitted ${event} event to ${roomId}`);
  }
};
...
function trackPerformance(req, res, next) {
  ...
  res.end = function(...args) {
    ...
    if (!performanceMetrics.endpoints[requestPath]) {
      performanceMetrics.endpoints[requestPath] = {
        count: 0,
        totalTime: 0,
    ...
    // Update metrics
    const metrics = performanceMetrics.endpoints[requestPath];
    metrics.count++;
    metrics.totalTime += responseTime;
    metrics.minTime = Math.min(metrics.minTime, responseTime);
    metrics.maxTime = Math.max(metrics.maxTime, responseTime);
    metrics.lastTime = responseTime;
    
    // If response is an error, record it
...
// Apply performance tracking middleware to all routes in this router
multiplierSimulatorRoute.use(trackPerformance);
...
class TradingSimulator {
  constructor(sessionId) {
    ...
    this.real_time_multiplier = 1/240;   // Speed up x6
    this.teams = {
      'A': {
    ...
    this.clientSyncStatus = {};
    this.simulation_start_time = Date.now();
    this.lastUpdateTime = Date.now();
  }
  
  // Synchronization method
  syncWithClient(clientTime, clientDisplayedTime) {
    const serverTime = Date.now();
    const latency = serverTime - this.lastUpdateTimestamp;
    
    // Calculate the difference between server and client time
    this.serverTimeOffset = serverTime - clientTime;
    
    // Record client sync status
    this.clientSyncStatus = {
      clientTime,
      clientDisplayedTime,
      serverTime,
      latency,
      serverDisplayedTime: this.current_time
    };
    
    // Calculate time drift between client and server
    const timeDrift = this.current_time - clientDisplayedTime;
    
    console.log(`Sync status: Client at ${clientDisplayedTime.toFixed(1)} min, Server at ${this.current_time.toFixed(1)} min, Drift: ${timeDrift.toFixed(1)} min, Latency: ${latency}ms`);
    
    // Record in performance metrics
    if (!performanceMetrics.clientSync[this.sessionId]) {
      performanceMetrics.clientSync[this.sessionId] = {
        lastSync: null,
        syncCount: 0,
        drifts: []
      };
    ...
    const clientMetrics = performanceMetrics.clientSync[this.sessionId];
    clientMetrics.lastSync = Date.now();
    clientMetrics.syncCount++;
    
    // Record time drift
    clientMetrics.drifts.push({
      timestamp: Date.now(),
      drift: timeDrift,
      latency
    });
    
    // Keep only the last 50 drift measurements
    if (clientMetrics.drifts.length > 50) {
      clientMetrics.drifts.shift();
    }
    
    // If client is significantly behind, help it catch up
    if (timeDrift > 1.0) {
      console.log(`Client significantly behind (${timeDrift.toFixed(1)} min). Sending catch-up data.`);
      
      // Return data needed for client to catch up
      return {
        catchUp: true,
        currentTimeIndex: this.displayed_time_index,
        currentTime: this.current_time,
        currentMultiplier: this.current_multiplier,
        collections: {
        ...
        recentHistory: this.history.slice(
          Math.max(0, this.displayed_time_index - 10), 
          this.displayed_time_index + 1
        )
      };
    }
    
    // Client is in sync or ahead
    return {
      catchUp: false,
      serverTime: this.current_time,
  ...
  generate_random_samples() {
    for (const resource of ['oxygen', 'lithium']) {
      for (const team of ['A', 'B']) {
        ...
        const deviations = Array.from({length: this.rounds}, () => randomNormal(0, 2));
        const samples = deviations.map(dev => Math.max(0, Math.round(mean_value + dev)));
        this.teams[team][resource] = samples;
        this.teams[team][`total_${resource}`] = samples.reduce((a, b) => a + b, 0);
      }
    }
  }
  ...
  display_initial_data() {
    ...
    table += "Round  A's Oxygen Crystals  A's Lithium Shards  B's Oxygen Crystals  B's Lithium Shards\n";
    ...
    const a_lithium_total = this.teams['A']['lithium'].reduce((a, b) => a + b, 0);
    const b_oxygen_total = this.teams['B']['oxygen'].reduce((a, b) => a + b, 0);
    const b_lithium_total = this.teams['B']['lithium'].reduce((a, b) => a + b, 0);
  ...
  generate_simulation_data() {
    ...
    const mean_A_oxygen = Math.floor(this.teams['A']['oxygen'].reduce((a, b) => a + b, 0) / this.rounds);
    const mean_A_lithium = Math.floor(this.teams['A']['lithium'].reduce((a, b) => a + b, 0) / this.rounds);
    const mean_B_oxygen = Math.floor(this.teams['B']['oxygen'].reduce((a, b) => a + b, 0) / this.rounds);
    const mean_B_lithium = Math.floor(this.teams['B']['lithium'].reduce((a, b) => a + b, 0) / this.rounds);
    ...
    for (let t = 0; t < this.simulation_steps; t++) {
      ...
      for (const resource in expected_intervals) {
        if (current_time >= next_collections[resource]) {
          ...
          next_collections[resource] += expected_intervals[resource] * (Math.random() * 1.5 + 0.25);
        }
        
        // Update expectation for this resource
        const remaining_time = this.simulation_time - current_time;
        expected_resources[resource] = collection_progress[resource] + 
          (remaining_time / this.simulation_time * expected_resources[resource]);
      }
      
      // Calculate predicted final multiplier
      const predicted_final_multiplier = Math.round(
        (expected_resources["A_oxygen"] + expected_resources["B_oxygen"]) * 
        (expected_resources["A_lithium"] + expected_resources["B_lithium"])
      );
      
      // Apply threshold to prevent case of very low multiplier
      let adjusted_predicted_multiplier = predicted_final_multiplier;
      if (predicted_final_multiplier < market_multiplier_threshold) {
        adjusted_predicted_multiplier = Math.max(
          Math.floor(Math.random() * (2 * market_multiplier_threshold - 1)) + 1,
          predicted_final_multiplier
        );
      }
      
      // Calculate current multiplier with deviation
      const predicted_dev = 0.15;
      const current_multiplier = Math.round(
        adjusted_predicted_multiplier * 
      ...
      side_trades = side_trades.filter(trade => {
        if (trade.expiry <= current_time) {
          ...
          this.side_trade_results[trade.id] = [trade.expression || trade.expr || trade.item, trade.price];
          return false;
        }
        return true;
      });
      ...
      this.simulation_data.push({
        ...
        predicted_multiplier: adjusted_predicted_multiplier,
        collections: { ...collections },
        side_trades: current_side_trades
      });
      
      this.multiplier_values.push(current_multiplier);
      ...
      console.log(`Generated data for time ${current_time.toFixed(1)} min: ${JSON.stringify(collections)}, Market Multiplier=${current_multiplier}`);
    }
    ...
    this.finalMultiplier = (collection_progress.A_oxygen + collection_progress.B_oxygen) * 
                      (collection_progress.A_lithium + collection_progress.B_lithium);
    
    console.log(`Pre-generated all simulation data. Final multiplier: ${this.finalMultiplier}`);
    return true;
  }
  
  // Improved method to start simulation with better timing
  async generate_live_collection() {
    ...
    // Use a more accurate timer approach
    const stepDuration = this.real_time_multiplier * 30 * 1000; // in milliseconds
    
    // Define the timer function
    const scheduleNextStep = () => {
      if (!this.active || this.paused) {
        // Reschedule if paused
        setTimeout(scheduleNextStep, 100);
        return;
      }
      
      // Calculate the elapsed real time
      const elapsedRealTime = Date.now() - this.simulation_start_time;
      
      // Calculate what the simulation time should be based on elapsed real time
      const expectedSimulationTime = (elapsedRealTime / 1000 / 60) / this.real_time_multiplier;
      
      // Calculate the expected display index
      const expectedIndex = Math.min(
        Math.floor(expectedSimulationTime * 2), // 2 steps per minute
        this.simulation_data.length - 1
      );
      
      // If we need to catch up, do it in larger steps
      if (expectedIndex > this.displayed_time_index + 1) {
        console.log(`Catching up from index ${this.displayed_time_index} to ${expectedIndex}`);
        
        // Fast-forward to catch up with expected time
        this.displayed_time_index = expectedIndex;
      } else if (expectedIndex > this.displayed_time_index) {
        // Normal step forward
        this.displayed_time_index++;
      }
      
      // Update current state from pre-generated data
      if (this.displayed_time_index < this.simulation_data.length) {
        ...
        setTimeout(scheduleNextStep, nextStepTime);
      } else {
        // Simulation complete
        this.finish();
      }
  ...
  generate_side_trade(mean_A_oxygen, mean_A_lithium, mean_B_oxygen, mean_B_lithium, current_time) {
    ...
    const expiry_time = Math.min(current_time + Math.floor(Math.random() * 11) + 5, this.simulation_time);
    ...
    const trade_price = Math.round(predicted_value * (Math.random() * 0.1 + 0.95));
    
    // Generate a unique trade ID (1-10000)
    const trade_id = Math.floor(Math.random() * 10000) + 1;
    
    console.log(`Generated Side Trade id ${trade_id}: ${expression} = ${trade_price} (Expires at ${expiry_time} min)`);
  ...
  async handleUserInput(input) {
    ...
    try {
      ...
      if (trade_input.length >= 2 && 
        ['m', 's'].includes(trade_input[0]) && 
        ['b', 's'].includes(trade_input[1])) {
          
        if (trade_input[0] === "m") {
          this.user_trades.push([trade_input, this.current_multiplier, this.current_time]);
          return { success: true, message: `Market ${trade_input[1] === 'b' ? 'buy' : 'sell'} at ${this.current_multiplier}` };
        }
        ...
        if (trade_input[0] === "s" && trade_input.length === 3) {
          ...
          return { success: true, message: `Side trade ${trade_input[1] === 'b' ? 'buy' : 'sell'} for trade ID ${id}` };
        }
      }
  ...
  show_results() {
    ...
    for (const [trade_input, trade_value, trade_time] of this.user_trades) {
      ...
      if (trade_type === 'm') {
        ...
        const [trade_expr, price] = this.side_trade_results[side_trade_id] || ["", 0];
      ...
      const profit = action === 'B' ? actual_value - trade_value : trade_value - actual_value;
      net_profit += profit;
      
      // Display trade details
      console.log(`${trade_type} ${action} at ${trade_time} min, Trade Price: ${trade_value}, Actual Price: ${actual_value}, Profit: ${profit}`);
    }
  ...
  processTrade(type, action, id = null) {
    ...
    if (type === 'm') {
      this.user_trades.push([trade_input, this.current_multiplier, this.current_time]);
      console.log(`Market ${action === 'b' ? 'buy' : 'sell'} at ${this.current_multiplier}`);
      return true;
    } else if (type === 's' && id) {
      // Find the side trade
      ...
      this.user_trades.push([trade_input, trade.price, this.current_time]);
      console.log(`Side trade ${action === 'b' ? 'buy' : 'sell'} for ID ${id} at price ${trade.price}`);
  ...
  // Get current data formatted for API response with enhanced timing information
  getCurrentData() {
    ...
    if (this.displayed_time_index === 0 && this.simulation_data.length > 0) {
      ...
      return {
        ...
        history: [{ time: initialData.time, multiplier: initialData.multiplier }],
        historical_collections: [{ time: initialData.time, collections: initialData.collections }],
        initial_data: this.initial_data,
        server_timestamp: Date.now(),
        simulation_speed: this.real_time_multiplier,
        display_index: this.displayed_time_index,
        total_steps: this.simulation_steps
      };
    }
    ...
    return {
      ...
      historical_collections: this.historical_collections.slice(0, this.displayed_time_index + 1),
      initial_data: this.initial_data,
      server_timestamp: Date.now(),
      simulation_speed: this.real_time_multiplier,
      display_index: this.displayed_time_index,
  ...
  // Method to manually set the displayed time index (for testing or admin purposes)
...
multiplierSimulatorRoute.post('/start_simulation', (req, res) => {
  ...
  try {
    ...
    // Emit via WebSocket that simulation has started
    emitToSimulation(req, sessionId, 'simulation-started', {
...
multiplierSimulatorRoute.post('/stop_simulation/:sessionId', (req, res) => {
  ...
  try {
    ...
    // Emit via WebSocket that simulation has paused
    emitToSimulation(req, fullSessionId, 'simulation-paused', {
...
multiplierSimulatorRoute.post('/resume_simulation/:sessionId', (req, res) => {
  ...
  try {
    ...
    // Emit via WebSocket that simulation has resumed
    emitToSimulation(req, fullSessionId, 'simulation-resumed', {
      session_id: fullSessionId,
      current_time: simulation.current_time,
      timestamp: Date.now()
    });
...
multiplierSimulatorRoute.post('/terminate_simulation/:sessionId', (req, res) => {
  const fullSessionId = `session:${req.params.sessionId}`;
  ...
  try {
    ...
    if (fields) {
      ...
      for (const field of fields) {
        ...
        } else if (field === 'current_' && 'current_time' in fullData && 'current_multiplier' in fullData) {
    ...
    responseData.server_timestamp = Date.now();
    
    // Emit data via WebSocket
    emitToSimulation(req, fullSessionId, 'simulation-data-update', {
      ...responseData,
      type: 'current_data'
    });
...
// New endpoint for client-server synchronization
multiplierSimulatorRoute.post('/sync/:sessionId', (req, res) => {
  const fullSessionId = `session:${req.params.sessionId}`;
  const { client_time, displayed_time } = req.body;
  
  if (!client_time || displayed_time === undefined) {
    return res.status(400).json({ error: 'client_time and displayed_time are required' });
  }
  ...
  try {
    ...
    const syncResult = simulation.syncWithClient(client_time, displayed_time);
    
    // Return sync data to client
    res.json({
      ...syncResult,
...
// New endpoint to get time range data (for catching up)
multiplierSimulatorRoute.get('/time_range/:sessionId', (req, res) => {
  const fullSessionId = `session:${req.params.sessionId}`;
  const startTime = parseFloat(req.query.start || '0');
  const endTime = parseFloat(req.query.end || '60');
  
  try {
    const simulation = getOrCreateSimulation(fullSessionId);
    
    // Filter history to the requested time range
    const filteredHistory = simulation.history.filter(
      entry => entry.time >= startTime && entry.time <= endTime
    );
    
    // Filter collections history
    const filteredCollections = simulation.historical_collections.filter(
      entry => entry.time >= startTime && entry.time <= endTime
    );
...
// Endpoint to manually set the displayed time (useful for testing or admin)
  ...
  try {
    ...
    if (type === 'm') {
      ...
      if (simulation.processTrade(type, action)) {
        ...
        emitToSimulation(req, fullSessionId, 'trade-submitted', {
          ...response,
          session_id: fullSessionId
        });
      ...
      if (!trade) {
        return res.status(404).json({ error: 'Side trade not found or expired' });
      }
      ...
      if (simulation.processTrade(type, action, id)) {
        ...
        const response = {
          ...
          trade: {
            ...
            price: tradeValue,
            time: simulation.current_time,
            expression: trade.expression || trade.expr || trade.item
          }
        };
...
multiplierSimulatorRoute.get('/results/:sessionId', (req, res) => {
  ...
  try {
    ...
    if (simulation.active && simulation.current_time < simulation.simulation_time) {
...
multiplierSimulatorRoute.get('/metrics', (req, res) => {
  res.json(performanceMetrics);
});

multiplierSimulatorRoute.get('/metrics/endpoints', (req, res) => {
  res.json(performanceMetrics.endpoints);
});

multiplierSimulatorRoute.get('/metrics/clients', (req, res) => {
  res.json(performanceMetrics.clientSync);
});

multiplierSimulatorRoute.get('/metrics/errors', (req, res) => {
  res.json(performanceMetrics.errors);
});
...
multiplierSimulatorRoute.get('/', (req, res) => {
  ...
  res.json({
    ...
    api_version: '1.1', // Updated version number
    active_sessions: Object.keys(sessions).length,
    server_time: Date.now()
  });
});

// New WebSocket-specific endpoint for simulation updates
multiplierSimulatorRoute.post('/subscribe/:sessionId', (req, res) => {
  const fullSessionId = `session:${req.params.sessionId}`;
  
  try {
    // Get or create simulation
    const simulation = getOrCreateSimulation(fullSessionId);
    const data = simulation.getCurrentData();
    
    // Send a success response with current data
    res.json({
      success: true,
    ...
    // Emit initial data event via WebSocket
    emitToSimulation(req, fullSessionId, 'simulation-data-update', {
      ...data,
      type: 'initial_data'
    });
